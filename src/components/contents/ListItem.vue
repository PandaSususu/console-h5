<template>
  <div>
    <ul>
      <li class="list-item" v-for="(item, index) in items" :key="'list' + index">
        <div class="pic" v-if="item.uid">
          <img :src="item.uid.pic" alt="" />
        </div>
        <div class="content">
          <div class="info">
            <p class="news-title">
              <span class="tag">{{ types[item.catalog] }}</span
              >{{ item.title }}
            </p>
            <div class="tags">
              <p class="user" v-if="item.uid">
                {{ item.uid.name
                }}<span v-if="item.uid.isVip === '1'"
              ...
              <span class="favs"
                ...
                >{{ item.favs }}</span
              >
              <span class="end tag">{{ item.isEnd === '0' ? '未结' : '已结' }}</span>
            </div>
          </div>
          <div class="label">
            <p class="labs">
              <span class="tag" v-for="(label, index) in item.tags" :key="'lable' + index" :class="label.class">{{ label.name }}</span>
            </p>
            <p class="count"><span class="layui-icon layui-icon-dialogue"></span>{{ item.answer }}</p>
          </div>
        </div>
      </li>
    </ul>
